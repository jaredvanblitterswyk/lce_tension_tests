...
#%% find points with high shear strain and high axial strain
...
if ~load_multiple_frames:
    ...
    ax = f.add_subplot(1,1,1)
    ax.scatter(strain_band_exx_df[['x_pix']], 
    ...
    ax.scatter(strain_band_exy_df[['x_pix']], 
    ...
    ax.scatter(excl_strain_band_df[['x_pix']], 
...
for i in range(1,num_strain_bands+1):
    ...
    axs[row,col].text(mask_frame*1.1, max_ylim*0.8,
                      'Mask frame: {:.0f}'.format(mask_frame),
                      fontsize = 4)
#%% - plot stress strain for each strain band on one plot
